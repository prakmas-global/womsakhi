...
export function CourseCard({ c, w }: { c: Course; w?: number }) {
  return (
    ...
    <Link href={`/app/programs/${c.id}`}
      ...
      <div className="p-3">
        <h3 className="line-clamp-2 text-[13px] font-semibold leading-snug" style={{ color: "var(--ux-ink)" }}>{c.title}</h3>
        ...
        {typeof c.pct === "number" ? (
          <div className="mt-2.5">
            <Progress pct={c.pct} />
            <p className="mt-1.5 text-end text-[11px] font-medium" style={{ color: "var(--ux-brand)" }}>{c.pct}% Complete</p>
          </div>
        ) : (
...
export function ResumeCard({ c }: { c: Course }) {
  return (
    <div className="ux-card flex gap-4 overflow-hidden" style={{ padding: 0 }}>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center pe-5">
        <h3 className="text-[16px] font-semibold" style={{ color: "var(--ux-ink)" }}>{c.title}</h3>
        ...
        <div className="mt-3.5 flex items-center gap-3">
          ...
          <span className="shrink-0 text-[11.5px] font-medium" style={{ color: "var(--ux-brand)" }}>{c.pct}% Complete</span>
        </div>
        <div className="mt-4">
          <Btn variant="primary" icon="Play">Resume Learning</Btn>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="ux-card flex w-[176px] shrink-0 items-start gap-2.5" style={{ padding: 13 }}>
      ...
      <div className="min-w-0">
        <h3 className="text-[12.5px] font-semibold leading-tight" style={{ color: "var(--ux-ink)" }}>{s.name}</h3>
        <p className="mt-1 text-[11px]" style={{ color: "var(--ux-muted)" }}>{s.level}</p>
      </div>
    </div>
  );
}
    ...
    <Card>
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11.5px]" style={{ color: "var(--ux-muted)" }}>
            Level {LEARNER.level} <span aria-hidden>•</span> {LEARNER.title}
          </p>
        </div>
      </div>
      <div className="mt-3"><Progress pct={pct} /></div>
      <p className="mt-1.5 text-end text-[11px]" style={{ color: "var(--ux-muted)" }}>
  ...
  return (
    <Card>
      <div className="flex items-center justify-between">
        <h2 className="flex items-center gap-2 text-[14.5px] font-semibold" style={{ color: "var(--ux-ink)" }}>
      ...
      <div className="mt-3.5 flex items-center gap-4">
        <div className="shrink-0 text-center">
          <p className="text-[26px] font-bold leading-none" style={{ color: "var(--ux-ink)" }}>{STREAK.days}</p>
          <p className="mt-0.5 text-[11px]" style={{ color: "var(--ux-muted)" }}>Days</p>
        </div>
        <div className="flex flex-1 justify-between">
          {STREAK.marks.map((on, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <span className="text-[10px]" style={{ color: "var(--ux-muted)" }}>{days[i]}</span>
      ...
      <div className="grid grid-cols-3 gap-2">
        {ACHIEVEMENTS.map((a) => (
          <div key={a.name} className="text-center">
            ...
            <p className="mt-1.5 text-[11px] font-semibold leading-tight" style={{ color: "var(--ux-ink)" }}>{a.name}</p>
            <p className="mt-0.5 text-[10px] leading-tight" style={{ color: "var(--ux-muted)" }}>{a.body}</p>
          </div>
        ))}
      </div>
    </Card>
    ...
    <Card>
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 text-[12px] leading-relaxed" style={{ color: "var(--ux-muted)" }}>
...
export function AskSakhiCard() {
  return (
    <div className="relative overflow-hidden rounded-[16px] p-[18px]"
      ...
      <h2 className="text-[15px] font-semibold" style={{ color: "var(--ux-ink)" }}>Hi, I&apos;m Sakhi! <span aria-hidden>👋</span></h2>
      <p className="mt-2 w-[76%] text-[12px] leading-relaxed" style={{ color: "var(--ux-ink-2)" }}>
