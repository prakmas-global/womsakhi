      ...
      */}
      <div className="grid items-stretch lg:grid-cols-[minmax(0,1fr)_auto]">
        <div className="p-6 sm:p-7">
          <h1 className="text-4xl font-extrabold leading-none tracking-[-0.03em]" style={{ color: v("--ux-ink") }}>
            Circle
          </h1>
          <p className="mt-2.5 max-w-[400px] text-smd leading-relaxed" style={{ color: v("--ux-ink-2") }}>
            A safe, supportive space for women to connect, ask, share and grow — together.
          </p>

          <form className="mt-5 flex max-w-[440px] items-center gap-2 rounded-full py-1.5 pe-1.5 ps-4"
            ...
            <input
              ...
              className="min-h-[38px] w-full min-w-0 bg-transparent text-xsm outline-none"
              style={{ color: v("--ux-ink") }}
            />
            <button type="submit" aria-label="Start this discussion"
                    className="ux-press ux-sq grid h-[36px] w-[36px] shrink-0 place-items-center rounded-full"
                    style={{ background: "linear-gradient(96deg, var(--ux-fill), var(--ux-fill-2))",
                             color: v("--ux-on-brand") }}>
              <Icons.ArrowRight className="h-[16px] w-[16px]" />
            </button>
          </form>
        </div>
...
export function TopicChips({ active, onPick, topics, counts }: {
  ...
  return (
    <div className="ux-noscroll mb-5 flex gap-2.5 overflow-x-auto pb-1">
      {all.map((t) => {
        const on = active === t.label;
        return (
          <button key={t.id} type="button" onClick={() => onPick(t.label)} aria-pressed={on}
                  className="ux-press ux-sq flex min-w-[104px] shrink-0 flex-col items-center gap-1.5 rounded-[14px] px-3.5 py-3"
            ...
            <span className="whitespace-nowrap text-2xs font-bold" style={{ color: v(on ? "--ux-brand" : "--ux-ink-2") }}>
              {t.label}
            </span>
            {counts[t.label] !== undefined && (
              <span className="text-3xs font-semibold" style={{ color: v("--ux-faint") }}>
                {counts[t.label]}
              </span>
            )}
          </button>
      ...
      <div className="grid gap-3.5" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(210px, 1fr))" }}>
        {rows.map((r) => (
          <Link key={r.id} href={r.href} className="ux-hov ux-sq flex flex-col rounded-[14px] p-4"
            ...
            <span className="inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1 text-3xs font-extrabold"
            ...
            <span className="mt-3.5 flex items-center gap-2">
              ...
              <span className="ms-auto flex items-center gap-1 text-2xs font-extrabold"
  ...
  return (
    <span className="flex shrink-0 items-center">
      {srcs.slice(0, 3).map((s, i) => (
        <span key={i} className="grid shrink-0 place-items-center overflow-hidden rounded-full text-3xs font-bold"
      ...
      <div className="grid grid-cols-3 gap-2">
        {cells.map((c) => (
          <div key={c.label} className="rounded-[12px] px-2 py-3 text-center"
            ...
            <span className="mt-1 block text-3xs font-semibold leading-tight" style={{ color: v("--ux-muted") }}>
              {c.label}
            </span>
          </div>
        ))}
...
export function CircleQuote() {
  return (
    <figure className="relative overflow-hidden rounded-[16px] p-[18px]"
      ...
      <figcaption className="mt-2 flex items-center gap-1.5 text-2xs font-semibold" style={{ color: v("--ux-muted") }}>
