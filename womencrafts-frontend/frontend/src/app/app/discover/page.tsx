...
function SectionHead({ icon: Icon, title, copy, href }: { icon: typeof Heart; title: string; copy: string; href: string }) {
  const tr = useT();
  return <header className={styles.sectionHead}><span><Icon /></span><div><h2>{title}</h2><p>{copy}</p></div><Link href={href}>{tr("calendar.viewAll")} <ArrowRight /></Link></header>;
}
...
export default function DiscoverPage() {
  ...
  return <HomeShell active="/app/discover" skeleton="grid" loadFailed="your recommendations"><main className={styles.page} data-dashboard="discover">
    ...
    <nav className={styles.filters} aria-label={tr("discover.recommendationFilters")}><div>{FILTERS.map(([id, label, Icon]) => <button type="button" key={id} aria-pressed={filter === id} onClick={() => setFilter(id)}><Icon />{label}</button>)}</div><button type="button" className={styles.filterButton}><SlidersHorizontal />Filters</button></nav>

    <section><SectionHead icon={Heart} title={tr("discover.opportunitiesForYou")} copy="Handpicked from your available recommendations." href="/app/explore" />
      {rails.source === "loading" ? <div className={styles.loading}>{tr("discover.findingTheBestMatchesForYou")}</div> : visible.length ? <div className={styles.opportunityGrid}>{visible.map((item, index) => <OpportunityCard key={`${item.kind}-${item.id}`} item={item} index={index} saved={saved.includes(item.id)} onSave={() => setSaved(current => current.includes(item.id) ? current.filter(id => id !== item.id) : [...current, item.id])} />)}</div> : <div className={styles.empty}>{tr("discover.noMatchesUnderThisFilterYet")} <button type="button" onClick={() => setFilter("all")}>{tr("books.showEverything")}</button></div>}
    </section>

    <section><SectionHead icon={UserRound} title={tr("discover.womenLikeYou2")} copy="Connect with women who share similar interests, skills or goals." href="/app/mentors" />
